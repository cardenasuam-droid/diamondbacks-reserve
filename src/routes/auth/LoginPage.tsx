  ...
  return (
    <div className="mx-auto max-w-sm space-y-6">
      ...
      {!isSupabaseConfigured && (
        <p className="rounded-lg border border-amber-500/30 bg-amber-500/15 p-3 text-sm text-amber-200">
  ...
  if (!selected) {
    return (
      <div className="space-y-3">
        ...
        />
        <ul className="max-h-80 divide-y divide-slate-100 overflow-y-auto rounded-lg border border-slate-200 bg-slate-100">
          {filtered.map((i) => (
            <li key={`${i.kind}-${i.id}`}>
              <button
                onClick={() => pick(i)}
...
function VerifyRegisterForm({
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      <p className="rounded-lg bg-brand-500/10 px-3 py-2 text-xs text-brand-200">{hint}</p>
