    ...
    <div className="flex flex-wrap gap-2">
      {list.map((t) => (
        <button
          ...
          onClick={() => onPick(t.id)}
          disabled={pending}
          className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-medium text-slate-800 shadow-sm ring-1 ring-slate-200 transition hover:ring-brand-400 disabled:opacity-50"
        >
          <span
